...
<!-- Image Description: This figure illustrates a time-series graph depicting a task learning scenario.  Nodes labeled 'a', 'b', and 'c' represent data points in different tasks over time (t-4 to t+1). The light blue area shows training data from two past tasks, while the pink area represents evaluation data for task 't'.  Dashed circles with question marks indicate unknown future data points. The graph demonstrates a temporal shift in data distribution, highlighting the transition between training and evaluation datasets and the challenge of predicting future outcomes. -->
...
## <span id="page-9-0"></span>2.2. Lifelong Learning
...
We explore the combination of lifelong learning on evolving graphs and new class detection on evolving graphs with imbalanced class distributions. In the following, we recapitulate the incremental training algorithm [\[13\]](#page-45-11), which we extend by a generic module for unseen class detection. Then, we introduce our gDOC method, an extension of DOC [\[17\]](#page-46-1) for unseen class detection, and show how it can be used in conjunction with incrementally trained graph neural networks.

### <span id="page-12-1"></span>3.1. Training Procedure for Lifelong Graph Learning

Our incremental training algorithm for GNNs is shown in Algorithm [1.](#page-13-0) We assume to have a sequence of T tasks T1, . . . , T<sup>T</sup> and a model f with parameters θ. Throughout the sequence of tasks, the graph changes in the sense that vertices and edges are inserted and deleted. Crucially, the new vertices can come with new classes that have not been part of the training data before. To address these changes, we use the incremental training procedure from our prior work [\[19\]](#page-46-3) for adapting neural networks to new tasks. As a preparation for task Tt, we retrain f on the labels of Tt−<sup>1</sup> to obtain θ (t) . Whenever l new classes appear in the training data, we add the corresponding number of parameters to the output layer of f (t) . Therefore, we have |θ (t) output weights| = |θ (t−1) output weights| + l · d<sup>h</sup> and |θ (t) output bias| = |θ (t−1) output bias| + l, where d<sup>h</sup> is the output layer size.

These parameters that model the new classes are randomly initialized. For the other parameters, we consider two options in our incremental training procedure: warm restarts and cold restarts. With cold restarts, we reinitialize θ (t) and retrain from scratch. On the contrary, when using warm restarts, we initialize the parameters for training on task T<sup>t</sup> with the final parameters of the previous task θ (t−1). Furthermore, we incorporate a generic module (lines 12–14) for unseen class detection in the incremental training algorithm. This operates on the logits of the final output layer and determines whether the classification of a particular vertex should be rejected because it belongs to a class that was not part of the training data.
...
<!-- Image Description: The image diagrams a graph neural network for time-series graph data.  A graph at time *t* (nodes 1-5 with edges) and its adjacency matrix are input, along with vertex features.  These are fed into a graph neural network, initialized with parameters from the previous time step (t-1). The network outputs logits, which are processed to produce predicted labels.  An out-of-distribution (OOD) detector then determines if the prediction is reliable or OOD, generating a final predicted label or OOD/ID. -->
...
### <span id="page-16-0"></span>4. Measure of k-Neighborhood Time Differences
...
<!-- Image Description: The image is a graph illustrating a time-series network.  Nodes represent time points (3, 4, 5), and edges show connections between them, labeled with time differences.  Edge colors and styles represent different types of relationships.  The  `tdiff₂(G)` set lists all calculated time differences between connected nodes. The graph likely demonstrates a specific algorithm or data structure used in the paper for temporal data analysis. -->
...
<!-- Image Description: The image displays nine plots visualizing network data across different years.  Three rows show the evolution of three aspects over time:  (1) Horizontal bar charts showing the count of nodes per year; (2) Log-log plots showing the degree distribution (frequency vs. degree) of the networks; and (3) Bar charts illustrating the distribution of node labels (classes) per year. The plots reveal changes in network size, connectivity structure, and class distributions over time. -->
...
<!-- Image Description: The image displays a line graph showing "drift magnitude" over time for three datasets: pharmabio, dblp-easy, and dblp-hard.  The y-axis represents drift magnitude (0-1), and the x-axis represents time (1999-2014).  Each dataset's drift magnitude fluctuates over time, with dblp-hard generally exhibiting higher magnitudes than the others. The graph likely illustrates concept drift in the context of the paper. -->
...
<!-- Image Description: The image displays three sets of box plots, each comparing the difference in timesteps for three values of "max hops" (1, 2, 3) across three datasets: DBLP-easy, DBLP-hard, and PharmaBio.  The plots visualize the distribution of timestep differences, showing median, quartiles, and outliers for each condition, allowing comparison of performance across datasets and hop parameters. -->
...
<!-- Image Description: The image displays six panels of line graphs, each showing model accuracy over epochs for different graph neural network (GNN) models (MLP, GCN, GCN-64, GraphSAGE, GAT)  on three datasets (cora, citeseer, pubmed) under two settings (A and B).  Shaded regions represent confidence intervals. The graphs compare model performance with and without pretraining (0 and 200 epochs).  The purpose is to illustrate the impact of different GNN architectures and pretraining on classification accuracy across various datasets and experimental settings. -->
...
<!-- Image Description: The image displays three line graphs comparing model accuracy over time for three datasets (dblp-easy, dblp-hard, pharmabio). Each graph shows accuracy trends for three models (mlp, gs-mean, gat) with and without incremental training, indicated by solid and dashed lines respectively.  The x-axis represents the year, and the y-axis represents accuracy. The purpose is to visually compare model performance across datasets and training methods. -->
...
### <span id="page-32-0"></span>8. Experiment 3: Lifelong Learning with Limited Labeled Data
...
<!-- Image Description: The image displays a line graph illustrating the accuracy of a model at different label rates.  Multiple lines represent varying "history" lengths (1, 3, 6, 25) and starting conditions ("cold" and "warm").  The graph shows how accuracy changes with increasing label rate for each history length and starting condition, allowing analysis of the model's performance under different data conditions. -->
...
<!-- Image Description: This bar chart displays the total number of nodes and unseen nodes for twelve tasks.  The light blue bars represent the total nodes per task, showing an increasing trend. The small red segments within each bar represent the number of unseen nodes, remaining relatively small compared to the total nodes across all tasks. The chart likely illustrates the scale of a data problem, showing a growing dataset with a consistently small portion of unseen data. -->
...
<!-- Image Description: The image displays a line graph showing the relationship between "global_mcc" (y-axis) and "doc_alpha" (x-axis), categorized by "doc_threshold" (different colored lines).  Each line represents a different doc_threshold value (0.0, 0.25, 0.5, 0.75, 1.0), illustrating how the global MCC metric varies with different alpha values under different thresholding.  Shaded areas represent confidence intervals. The graph likely assesses the performance of a model across various parameter settings within the context of the paper. -->
...
To understand this result, we recall that risk reduction is a technique for calculating class-specific thresholds τ<sup>i</sup> (see Section [3.2\)](#page-13-1). However, this is only possible up to the global minimum threshold τ . Thus, even with risk reduction, class-specific thresholds cannot go below τ .

### <span id="page-40-0"></span>9.5. Combining gDOC with Different GNN Base Models
...
We have shown in Section [8](#page-32-0) that the methods work well, even when the labeled data are limited, which is essential for real-world applications because data annotation is expensive.

With the introduction of gDOC, we have made the first step to introduce new class detection in lifelong graph learning in Section [9,](#page-34-0) by combining graph neural networks with the DOC [\[17\]](#page-46-1) module and extending it to take into account class imbalance. Our experiments on new class detection show that it is necessary to adjust the weights of binary cross-entropy training in gDOC to account for the imbalanced label distribution. Contrary to the original DOC, we have not observed any improvements with risk reduction through the standard deviation of logits. Instead, the best results were achieved with an appropriate threshold (τ = 0.75) regardless of the risk reduction factor α. We acknowledge that emitting a crisp decision in unsupervised unseen class detection is a highly challenging problem.
...
- <span id="page-45-0"></span>[2] F. Scarselli, M. Gori, A. C. Tsoi, M. Hagenbuchner, G. Monfardini, The graph neural network model, IEEE Trans. Neural Networks 20 (1). [doi:](http://dx.doi.org/10.1109/TNN.2008.2005605) [10.1109/TNN.2008.2005605](http://dx.doi.org/10.1109/TNN.2008.2005605).
- <span id="page-45-1"></span>[3] T. N. Kipf, M. Welling, Semi-supervised classification with graph convolutional networks, in: ICLR, 2017.
- <span id="page-45-2"></span>[4] W. L. Hamilton, Z. Ying, J. Leskovec, Inductive representation learning on large graphs, in: NeurIPS, 2017.
- <span id="page-45-3"></span>[5] P. Veliˇckovi´c, G. Cucurull, A. Casanova, A. Romero, P. Li`o, Y. Bengio, Graph attention networks, in: ICLR, 2018.
- <span id="page-45-4"></span>[6] P. Sen, G. Namata, M. Bilgic, L. Getoor, B. Gallagher, T. Eliassi-Rad, Collective classification in network data, AI Magazine 29 (3).
- <span id="page-45-5"></span>[7] S. Thrun, Lifelong learning algorithms, in: Learning to learn, Springer, 1998, pp. 181–209.
- <span id="page-45-6"></span>[8] Z. Chen, B. Liu, Lifelong Machine Learning, Second Edition, Synthesis Lectures on Artificial Intelligence and Machine Learning, Morgan & Claypool Publishers, 2018. [doi:10.2200/S00832ED1V01Y201802AIM037](http://dx.doi.org/10.2200/S00832ED1V01Y201802AIM037).
- <span id="page-45-7"></span>[9] G. I. Parisi, R. Kemker, J. L. Part, C. Kanan, S. Wermter, Continual lifelong learning with neural networks: A review, Neural Networks 113 (2019) 54–71. [doi:10.1016/j.neunet.2019.01.012](http://dx.doi.org/10.1016/j.neunet.2019.01.012).
- <span id="page-45-8"></span>[10] J. Wang, G. Song, Y. Wu, L. Wang, Streaming graph neural networks via continual learning, in: Proceedings of the 29th ACM International Conference on Information & Knowledge Management, CIKM '20, Association for Computing Machinery, 2020, pp. 1515–1524. [doi:10.1145/3340531.](http://dx.doi.org/10.1145/3340531.3411963) [3411963](http://dx.doi.org/10.1145/3340531.3411963).
- <span id="page-45-9"></span>[11] F. Zhou, C. Cao, Overcoming Catastrophic Forgetting in Graph Neural Networks with Experience Replay (2021). [arXiv:2003.09908](http://arxiv.org/abs/2003.09908).
- <span id="page-45-10"></span>[12] C. Wang, Y. Qiu, D. Gao, S. Scherer, Lifelong Graph Learning, in: Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition, 2022, pp. 13719–13728.
- <span id="page-45-11"></span>[13] L. Galke, B. Franke, T. Zielke, A. Scherp, Lifelong learning of graph neural networks for open-world node classification, in: IJCNN, IEEE, 2021. [doi:](http://dx.doi.org/10.1109/IJCNN52387.2021.9533412) [10.1109/IJCNN52387.2021.9533412](http://dx.doi.org/10.1109/IJCNN52387.2021.9533412).
- <span id="page-45-12"></span>[14] X. Chen, J. Wang, K. Xie, TrafficStream: A Streaming Traffic Flow Forecasting Framework Based on Graph Neural Networks and Continual Learning (2021). [arXiv:2106.06273](http://arxiv.org/abs/2106.06273).
- <span id="page-45-13"></span>[15] F. G. Febrinanto, F. Xia, K. Moore, C. Thapa, C. Aggarwal, Graph Lifelong Learning: A Survey (2022). [arXiv:2202.10688](http://arxiv.org/abs/2202.10688).

- <span id="page-46-0"></span>[16] J. Cai, X. Wang, C. Guan, Y. Tang, J. Xu, B. Zhong, W. Zhu, Multimodal continual graph learning with neural architecture search, in: WWW, ACM, 2022, pp. 1292–1300. [doi:10.1145/3485447.3512176](http://dx.doi.org/10.1145/3485447.3512176).
- <span id="page-46-1"></span>[17] L. Shu, H. Xu, B. Liu, DOC: Deep open classification of text documents, in: EMNLP, ACL, 2017, pp. 2911–2916. [doi:10.18653/v1/d17-1314](http://dx.doi.org/10.18653/v1/d17-1314).
- <span id="page-46-2"></span>[18] S. Thrun, T. M. Mitchell, Learning one more thing, in: IJCAI, Morgan Kaufmann, 1995, pp. 1217–1225.
- <span id="page-46-3"></span>[19] L. Galke, I. Vagliano, A. Scherp, Can graph neural networks go "online"? An analysis of pretraining and inference, in: Representation Learning on Graphs and Manifolds, ICLR Workshop, 2019.
- <span id="page-46-4"></span>[20] V. P. Dwivedi, C. K. Joshi, T. Laurent, Y. Bengio, X. Bresson, Benchmarking graph neural networks (2020). [arXiv:2003.00982](http://arxiv.org/abs/2003.00982).
- <span id="page-46-5"></span>[21] Z. Ying, J. You, C. Morris, X. Ren, W. L. Hamilton, J. Leskovec, Hierarchical graph representation learning with differentiable pooling, in: NeurIPS, 2018.
- <span id="page-46-6"></span>[22] K. Xu, W. Hu, J. Leskovec, S. Jegelka, How powerful are graph neural networks?, in: ICLR, 2019.
- <span id="page-46-7"></span>[23] X. Bresson, T. Laurent, Residual Gated Graph ConvNets (2017). [arXiv:](http://arxiv.org/abs/1711.07553) [1711.07553](http://arxiv.org/abs/1711.07553).
- <span id="page-46-8"></span>[24] F. Monti, D. Boscaini, J. Masci, E. Rodol`a, J. Svoboda, M. M. Bronstein, Geometric deep learning on graphs and manifolds using mixture model CNNs, in: CVPR, IEEE, 2017.
- <span id="page-46-9"></span>[25] F. Wu, A. H. S. Jr., T. Zhang, C. Fifty, T. Yu, K. Q. Weinberger, Simplifying graph convolutional networks, in: ICML, PMLR, 2019, pp. 6861–6871.
- <span id="page-46-10"></span>[26] E. Rossi, F. Frasca, B. Chamberlain, D. Eynard, M. M. Bronstein, F. Monti, SIGN: scalable inception graph neural networks (2020). [arXiv:2004.](http://arxiv.org/abs/2004.11198) [11198](http://arxiv.org/abs/2004.11198).
- <span id="page-46-11"></span>[27] A. Bojchevski, J. Klicpera, B. Perozzi, A. Kapoor, M. Blais, B. R´ozemberczki, M. Lukasik, S. G¨unnemann, Scaling graph neural networks with approximate pagerank, in: KDD, ACM, 2020, pp. 2464–2473.
- <span id="page-46-12"></span>[28] Y. Hu, H. You, Z. Wang, Z. Wang, E. Zhou, Y. Gao, Graph-MLP: Node classification without message passing in graph (2021). [arXiv:2106.04051](http://arxiv.org/abs/2106.04051).
- <span id="page-46-13"></span>[29] X. He, K. Deng, X. Wang, Y. Li, Y. Zhang, M. Wang, LightGCN: Simplifying and Powering Graph Convolution Network for Recommendation, in: Proceedings of the 43rd International ACM SIGIR Conference on Research and Development in Information Retrieval, SIGIR '20, Association for Computing Machinery, 2020, pp. 639–648. [doi:10.1145/3397271.](http://dx.doi.org/10.1145/3397271.3401063) [3401063](http://dx.doi.org/10.1145/3397271.3401063).

- <span id="page-47-0"></span>[30] W. Huang, T. Zhang, Y. Rong, J. Huang, Adaptive sampling towards fast graph representation learning, in: NeurIPS, 2018.
- <span id="page-47-1"></span>[31] J. Chen, T. Ma, C. Xiao, FastGCN: Fast learning with graph convolutional networks via importance sampling, in: International Conference on Learning Representations, 2018.
- <span id="page-47-2"></span>[32] W. Chiang, X. Liu, S. Si, Y. Li, S. Bengio, C. Hsieh, Cluster-gcn: An efficient algorithm for training deep and large graph convolutional networks, in: KDD, ACM, 2019, pp. 257–266.
- <span id="page-47-3"></span>[33] H. Zeng, H. Zhou, A. Srivastava, R. Kannan, V. K. Prasanna, Graphsaint: Graph sampling based inductive learning method, in: ICLR, 2020.
- <span id="page-47-4"></span>[34] K. Xu, C. Li, Y. Tian, T. Sonobe, K. Kawarabayashi, S. Jegelka, Representation learning on graphs with jumping knowledge networks, in: ICML, 2018.
- <span id="page-47-5"></span>[35] E. Rossi, B. Chamberlain, F. Frasca, D. Eynard, F. Monti, M. M. Bronstein, Temporal graph networks for deep learning on dynamic graphs (2020). [arXiv:2006.10637](http://arxiv.org/abs/2006.10637).
- <span id="page-47-6"></span>[36] G. H. Nguyen, J. B. Lee, R. A. Rossi, N. K. Ahmed, E. Koh, S. Kim, Continuous-time dynamic network embeddings, in: WWW, ACM, 2018, pp. 969–976.
- <span id="page-47-7"></span>[37] J. B. Lee, G. Nguyen, R. A. Rossi, N. K. Ahmed, E. Koh, S. Kim, Dynamic node embeddings from edge streams, IEEE Trans. Emerg. Top. Comput. Intell. 5 (6) (2021) 931–946. [doi:10.1109/TETCI.2020.3011432](http://dx.doi.org/10.1109/TETCI.2020.3011432).
- <span id="page-47-8"></span>[38] P. Goyal, N. Kamra, X. He, Y. Liu, DynGEM: Deep embedding method for dynamic graphs (2018). [arXiv:1805.11273](http://arxiv.org/abs/1805.11273).
- <span id="page-47-9"></span>[39] P. Goyal, S. R. Chhetri, A. Canedo, dyngraph2vec: Capturing network dynamics using dynamic graph representation learning, Knowl. Based Syst. 187. [doi:10.1016/j.knosys.2019.06.024](http://dx.doi.org/10.1016/j.knosys.2019.06.024).
- <span id="page-47-10"></span>[40] R. Trivedi, H. Dai, Y. Wang, L. Song, Know-Evolve: Deep temporal reasoning for dynamic knowledge graphs, in: ICML, PMLR, 2017.
- <span id="page-47-11"></span>[41] Y. Seo, M. Defferrard, P. Vandergheynst, X. Bresson, Structured sequence modeling with graph convolutional recurrent networks, in: ICONIP, Springer, 2018.
- <span id="page-47-12"></span>[42] S. Kumar, X. Zhang, J. Leskovec, Learning dynamic embeddings from temporal interactions (2018). [arXiv:1812.02289](http://arxiv.org/abs/1812.02289).
- <span id="page-47-13"></span>[43] R. Trivedi, M. Farajtabar, P. Biswal, H. Zha, Dyrep: Learning representations over dynamic graphs, in: ICLR, 2019.

- <span id="page-48-0"></span>[44] F. Manessi, A. Rozza, M. Manzo, Dynamic graph convolutional networks, Pattern Recognition 97.
- <span id="page-48-1"></span>[45] A. Sankar, Y. Wu, L. Gou, W. Zhang, H. Yang, DySAT: Deep neural representation learning on dynamic graphs via self-attention networks, in: WSDM, ACM, 2020.
- <span id="page-48-2"></span>[46] A. Pareja, G. Domeniconi, J. Chen, T. Ma, T. Suzumura, H. Kanezashi, T. Kaler, T. B. Schardl, C. E. Leiserson, Evolvegcn: Evolving graph convolutional networks for dynamic graphs, in: AAAI, AAAI Press, 2020, pp. 5363–5370.
- <span id="page-48-3"></span>[47] X. Da, R. Chuanwei, K. Evren, K. Sushant, A. Kannan, Inductive representation learning on temporal graphs, in: ICLR, 2020.
- <span id="page-48-4"></span>[48] D. Lopez-Paz, M. Ranzato, Gradient episodic memory for continual learning, in: NIPS, 2017, pp. 6467–6476.
- <span id="page-48-5"></span>[49] D. L. Silver, Q. Yang, L. Li, Lifelong machine learning systems: Beyond learning algorithms, in: AAAI Spring Symposium: Lifelong Machine Learning, Vol. SS-13-05, AAAI, 2013.
- <span id="page-48-6"></span>[50] B. Liu, Lifelong machine learning: a paradigm for continuous learning, Frontiers Comput. Sci. 11 (3) (2017) 359–361. [doi:10.1007/](http://dx.doi.org/10.1007/s11704-016-6903-6) [s11704-016-6903-6](http://dx.doi.org/10.1007/s11704-016-6903-6).
- <span id="page-48-7"></span>[51] M. Herbster, M. Pontil, L. Wainer, Online learning over graphs, in: ICML, Vol. 119, ACM, 2005, pp. 305–312.
- <span id="page-48-8"></span>[52] P. Ruvolo, E. Eaton, ELLA: An efficient lifelong learning algorithm, in: ICML, JMLR.org, 2013, pp. 507–515.
- <span id="page-48-9"></span>[53] G. Fei, S. Wang, B. Liu, Learning cumulatively to become more knowledgeable, in: KDD, ACM, 2016, pp. 1565–1574.
- <span id="page-48-10"></span>[54] A. V. Robins, Catastrophic forgetting, rehearsal and pseudorehearsal, Connect. Sci. 7 (2) (1995) 123–146.
- <span id="page-48-11"></span>[55] B. Wang, Y. Chen, X. Li, J. Chen, Lifelong classification in open world with limited storage requirements, Neural Comput. 33 (7) (2021) 1818– 1852. [doi:10.1162/neco\\\_a\\\_01391](http://dx.doi.org/10.1162/neco_a_01391).
- <span id="page-48-12"></span>[56] S.-A. Rebuffi, A. Kolesnikov, G. Sperl, C. H. Lampert, icarl: Incremental classifier and representation learning, in: CVPR, 2017.
- <span id="page-48-13"></span>[57] R. M. French, Catastrophic forgetting in connectionist networks, Trends in Cognitive Sciences 3 (4) (1999) 128–135. [doi:10.1016/S1364-6613\(99\)](http://dx.doi.org/10.1016/S1364-6613(99)01294-2) [01294-2](http://dx.doi.org/10.1016/S1364-6613(99)01294-2).

- <span id="page-49-0"></span>[58] J. Devlin, M. Chang, K. Lee, K. Toutanova, BERT: pre-training of deep bidirectional transformers for language understanding, in: NAACL-HLT, Association for Computational Linguistics, 2019.
- <span id="page-49-1"></span>[59] A. Dosovitskiy, L. Beyer, A. Kolesnikov, D. Weissenborn, X. Zhai, T. Unterthiner, M. Dehghani, M. Minderer, G. Heigold, S. Gelly, J. Uszkoreit, N. Houlsby, An image is worth 16x16 words: Transformers for image recognition at scale, in: ICLR, OpenReview.net, 2021.
- <span id="page-49-2"></span>[60] H. Liu, Y. Yang, X. Wang, Overcoming Catastrophic Forgetting in Graph Neural Networks, Proceedings of the AAAI Conference on Artificial Intelligence 35 (10) (2021) 8653–8661. [doi:10.1609/aaai.v35i10.17049](http://dx.doi.org/10.1609/aaai.v35i10.17049).
- <span id="page-49-3"></span>[61] Z. Tan, K. Ding, R. Guo, H. Liu, Graph Few-shot Class-incremental Learning, in: Proceedings of the Fifteenth ACM International Conference on Web Search and Data Mining, WSDM '22, Association for Computing Machinery, New York, NY, USA, 2022, pp. 987–996. [doi:10.1145/3488560.](http://dx.doi.org/10.1145/3488560.3498455) [3498455](http://dx.doi.org/10.1145/3488560.3498455).
- <span id="page-49-4"></span>[62] S. Liang, Y. Li, R. Srikant, Enhancing the reliability of out-of-distribution image detection in neural networks, in: ICLR, 2018.
- <span id="page-49-5"></span>[63] K. Lee, K. Lee, H. Lee, J. Shin, A simple unified framework for detecting out-of-distribution samples and adversarial attacks, in: NeurIPS, 2018, pp. 7167–7177.
- <span id="page-49-6"></span>[64] D. Macˆedo, T. I. Ren, C. Zanchettin, A. L. Oliveira, T. Ludermir, Entropic out-of-distribution detection, in: IJCNN, IEEE, 2021.
- <span id="page-49-7"></span>[65] D. Macˆedo, T. Ludermir, Improving entropic out-of-distribution detection using isometric distances and the minimum distance score (2021). [arXiv:](http://arxiv.org/abs/2105.14399) [2105.14399](http://arxiv.org/abs/2105.14399).
- <span id="page-49-8"></span>[66] A. R. Dhamija, M. G¨unther, T. E. Boult, Reducing network agnostophobia, in: NeurIPS, 2018, pp. 9175–9186.
- <span id="page-49-9"></span>[67] D. Hendrycks, M. Mazeika, T. G. Dietterich, Deep anomaly detection with outlier exposure, in: ICLR, OpenReview.net, 2019.
- <span id="page-49-10"></span>[68] J. Yang, K. Zhou, Y. Li, Z. Liu, Generalized out-of-distribution detection: A survey (2021). [arXiv:2110.11334](http://arxiv.org/abs/2110.11334).
- <span id="page-49-11"></span>[69] G. Pang, C. Shen, L. Cao, A. van den Hengel, Deep learning for anomaly detection: A review, ACM Comput. Surv. 54 (2) (2021) 38:1–38:38. [doi:](http://dx.doi.org/10.1145/3439950) [10.1145/3439950](http://dx.doi.org/10.1145/3439950).
- <span id="page-49-12"></span>[70] M. M. Masud, J. Gao, L. Khan, J. Han, B. M. Thuraisingham, Classification and novel class detection in concept-drifting data streams under time constraints, IEEE Trans. Knowl. Data Eng. 23 (6) (2011) 859–874. [doi:10.1109/TKDE.2010.61](http://dx.doi.org/10.1109/TKDE.2010.61).

- <span id="page-50-0"></span>[71] A. Bendale, T. E. Boult, Towards open set deep networks, in: CVPR, IEEE, 2016, pp. 1563–1572. [doi:10.1109/CVPR.2016.173](http://dx.doi.org/10.1109/CVPR.2016.173).
- <span id="page-50-1"></span>[72] M. Wu, S. Pan, X. Zhu, OpenWGL: Open-world graph learning, in: ICDM, IEEE, 2020, pp. 681–690.
- <span id="page-50-2"></span>[73] H. Xu, B. Liu, L. Shu, P. Yu, Open-world Learning and Application to Product Classification, in: The World Wide Web Conference on - WWW '19, ACM Press, San Francisco, CA, USA, 2019, pp. 3413–3419. [doi:](http://dx.doi.org/10.1145/3308558.3313644) [10.1145/3308558.3313644](http://dx.doi.org/10.1145/3308558.3313644).
- <span id="page-50-3"></span>[74] Y. S. Aurelio, G. M. de Almeida, C. L. Castro, A. de P´adua Braga, Learning from imbalanced data sets with weighted cross-entropy function, Neural Process. Lett. 50 (2) (2019) 1937–1949. [doi:10.1007/](http://dx.doi.org/10.1007/s11063-018-09977-1) [s11063-018-09977-1](http://dx.doi.org/10.1007/s11063-018-09977-1).
- <span id="page-50-4"></span>[75] C. Aggarwal, K. Subbian, Evolutionary network analysis: A survey, ACM Comput. Surv. 47 (1). [doi:10.1145/2601412](http://dx.doi.org/10.1145/2601412).
- <span id="page-50-5"></span>[76] J. Chen, J. Zhu, L. Song, Stochastic training of graph convolutional networks with variance reduction, in: ICML, 2018.
- <span id="page-50-6"></span>[77] W. Hu, M. Fey, M. Zitnik, Y. Dong, H. Ren, B. Liu, M. Catasta, J. Leskovec, Open graph benchmark: Datasets for machine learning on graphs, in: NeurIPS, 2020.
- <span id="page-50-7"></span>[78] Z. Yang, W. W. Cohen, R. Salakhutdinov, Revisiting semi-supervised learning with graph embeddings, in: ICML, 2016.
- <span id="page-50-8"></span>[79] L. Galke, F. Mai, I. Vagliano, A. Scherp, Multi-modal adversarial autoencoders for recommendations of citations and subject labels, in: 26th Conference on User Modeling, Adaptation and Personalization, ACM, 2018. [doi:10.1145/3209219.3209236](http://dx.doi.org/10.1145/3209219.3209236).
- <span id="page-50-9"></span>[80] M. E. Newman, Power laws, Pareto distributions and Zipf's law, Contemporary physics 46 (5).
- <span id="page-50-10"></span>[81] G. I. Webb, R. Hyde, H. Cao, H. L. Nguyen, F. Petitjean, Characterizing concept drift, Data Mining and Knowledge Discovery 30 (4) (2016) 964– 994.
- <span id="page-50-11"></span>[82] G. I. Webb, L. K. Lee, B. Goethals, F. Petitjean, Analyzing concept drift and shift from sample data, Data Mining and Knowledge Discovery 32 (5) (2018) 1179–1199.
- <span id="page-50-12"></span>[83] X. Glorot, Y. Bengio, Understanding the difficulty of training deep feedforward neural networks, in: AISTATS, JMLR.org, 2010.
- <span id="page-50-13"></span>[84] D. P. Kingma, J. Ba, Adam: A method for stochastic optimization, in: ICLR, 2015.

- <span id="page-51-0"></span>[85] J. Lin, Divergence measures based on the Shannon entropy, IEEE Trans. Information Theory 37 (1). [doi:10.1109/18.61115](http://dx.doi.org/10.1109/18.61115).
- <span id="page-51-1"></span>[86] M. Wang, et al., Deep Graph Library: Towards efficient and scalable deep learning on graphs (2019). [arXiv:1909.01315](http://arxiv.org/abs/1909.01315).
- <span id="page-51-2"></span>[87] M. Fey, J. E. Lenssen, Fast graph representation learning with PyTorch Geometric, in: ICLR Workshop on Representation Learning on Graphs and Manifolds, 2019.
- <span id="page-51-3"></span>[88] I. J. Goodfellow, Y. Bengio, A. C. Courville, Deep Learning, Adaptive computation and machine learning, MIT Press, 2016.
- <span id="page-51-4"></span>[89] F. Mai, L. Galke, A. Scherp, Using deep learning for title-based semantic subject indexing to reach competitive performance to full-text, in: Proceedings of the 18th ACM/IEEE on Joint Conference on Digital Libraries, JCDL 2018, ACM, 2018. [doi:10.1145/3197026.3197039](http://dx.doi.org/10.1145/3197026.3197039).
- <span id="page-51-5"></span>[90] D. Chicco, G. Jurman, The advantages of the Matthews correlation coefficient (MCC) over F1 score and accuracy in binary classification evaluation, BMC genomics 21 (1) (2020) 1–13.