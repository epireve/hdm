...
<!-- Image Description: This diagram illustrates a neural network architecture for multi-label text classification. Word and label tokens are processed by a transformer layer, generating document and label embeddings. These embeddings undergo a contrastive learning process, shown as interconnected nodes, enhancing label representations. Finally, a multi-layer perceptron (MLP) produces the classification output. The diagram uses color-coded lines to represent different embedding types and their interactions within the model. -->
...
<!-- Image Description: This image displays three bar charts (a, b, c) comparing the performance of three distance metrics (cosine, Euclidean, Manhattan) in a retrieval task. Each chart shows precision at a different recall level (P@1, P@3, P@5), representing the proportion of relevant items retrieved within the top 1, 3, and 5 results, respectively. The charts illustrate the relative effectiveness of each distance metric for different recall levels. -->
...
<!-- Image Description: The image contains three bar charts (a, b, c) comparing four models (A, A/c, A/e, A/v) based on their performance metrics: Precision@1 (P@1), Precision@3 (P@3), and Precision@5 (P@5). Each chart displays the precision scores for the different models, allowing for a visual comparison of their ranking performance at different cutoff points. The purpose is to present a quantitative evaluation of the models' effectiveness. -->
...
<!-- Image Description: The image contains three line graphs (a, b, c) showing model performance (P@1, P@3, P@5) over 100 epochs. Each graph compares four models (A, A/c, A/e, A/v). The y-axis represents precision at k (P@k), and the x-axis represents the number of training epochs. The graphs illustrate the training progress and the comparative performance of different model variations. -->
