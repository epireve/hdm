...
<!-- Image Description: This image presents four histograms comparing the distribution of "Relatedness Scores" for two methods (Conceptual Captions and Ngram Image Search) across two data sources (ICE-Titles and ICE-Comments). Each histogram shows the frequency of different relatedness scores, revealing the distribution's shape and potentially indicating the effectiveness of each method in different contexts. The y-axis represents the number of examples (in thousands), and the x-axis represents the relatedness score. -->
...
<!-- Image Description: The image displays four histograms comparing the quality scores of image captions generated by two methods: ConceptualCaptions and Ngram Image Search. Each histogram shows the distribution of quality scores for titles (ICE-Title) and comments (ICE-Comments*) from each method. The x-axis represents the quality score (0-3), and the y-axis shows the number of examples in thousands. The histograms illustrate the performance differences between the two captioning methods in terms of caption quality. -->
...
<!-- Image Description: The image displays a bar graph showing the relationship between "Relatedness" and a "Normalized Score." Each bar represents a different level of relatedness (numerical values on the x-axis), and its height indicates the corresponding normalized score (0-1 on the y-axis). Error bars show variability. The graph likely illustrates the performance or correlation of a system or model based on the degree of relatedness between data points, demonstrating that higher relatedness tends to correlate with higher scores. -->
...
<!-- Image Description: The bar graph displays the relationship between "Quality" (x-axis) and "Normalized Score" (y-axis). Each bar represents a different quality level, showing the mean normalized score and error bars indicating variability. The graph suggests a positive correlation: higher quality levels generally correspond to higher normalized scores. The purpose is to visually represent and compare the performance at different quality levels, likely to demonstrate the impact of quality on a specific metric in the study. -->
