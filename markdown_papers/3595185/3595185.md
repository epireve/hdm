...
authors: "Xiaojin Zhang, Yan Kang, Kai Chen, Lixin Fan, Qiang Yang. 2023. Trading Off Privacy, Utility"
year: 2016
doi: "10.1145/3595185"
...
<!-- Image Description: This image presents two diagrams illustrating a federated learning system. The left diagram shows a standard system where multiple clients (1 to K) send their local model parameters (θ<sup>c</sup>) to a server, which aggregates them (θ<sup>s</sup><sub>agg</sub> = Aggregate(θ<sup>s</sup><sub>1</sub>,...,θ<sup>s</sup><sub>K</sub>)) and sends back the updated global parameters. The right diagram depicts an adversarial attack scenario, where an adversarial client manipulates its parameters (θ<sup>s</sup><sub>1,b</sub>) to influence the global model, impacting the labeled client's parameter updates (θ<sup>o</sup><sub>2,b</sub>). The diagrams use boxes to represent parameters and arrows to show data flow, clarifying the process and adversarial manipulation. -->
...
<!-- Image Description: This flowchart illustrates a federated learning (FL) system's security. Data (D) is processed through a protector and adversary. `msg<sup>o</sup>` and `msg<sup>s</sup>` represent original and protected messages, respectively, with `W<sup>o</sup>` and `W<sup>s</sup>` being the resulting weights. The diagram shows the impact of attacks, comparing FL with and without protection, quantifying privacy leakage (ε<sub>p</sub>), utility loss (ε<sub>u</sub>), and efficiency reduction (ε<sub>e</sub>). The purpose is to demonstrate the system's vulnerability and the effectiveness of the proposed protection mechanism. -->
...
<!-- Image Description: The image displays a graph showing a Gaussian probability density function \(P_{W_{fed}}^S(w)\). Two shaded vertical bars represent the cumulative probability density of near-optimal parameters, denoted as \(W_\Delta\), symmetrically positioned around the peak of the curve. The graph illustrates the concentration of near-optimal parameters within a specific range, providing a visual representation of parameter sensitivity and the distribution of successful solutions in the paper's context. -->
...
<!-- Image Description: The image displays a Venn diagram and a probability density function graph. The Venn diagram shows a set $\mathcal{W}_k^S$ partitioned into two subsets, $\mathcal{W}_{k,\Xi}^+$ (blue) and $\mathcal{W}_{k,\Xi}^-$ (red), by a line representing $\mathcal{U}_k$. The graph plots a probability density function $p_{\mathcal{W}_k}^S(w)$, with areas under the curve corresponding to $\mathcal{W}_{k,\Xi}^+$ and $\mathcal{W}_{k,\Xi}^-$ shaded blue and red respectively. The image illustrates a probability distribution split into two regions based on a threshold, likely for a statistical analysis within the paper. -->
...
<!-- Image Description: The image is a triangular diagram illustrating the No-Free-Lunch Theorem. The vertices represent error measures (ε<sub>u</sub>, ε<sub>p</sub>, ε<sub>e</sub>), and the sides display inequalities involving these errors and constants (C<sub>d</sub>, C<sub>x</sub>, C<sub>1</sub>). The central inequality, ε<sub>p</sub> + (C<sub>d</sub>/2)⋅ε<sub>u</sub> + (C<sub>x</sub>/2)⋅ε<sub>e</sub> ≥ C<sub>1</sub>, represents a constraint within the theorem's context. The diagram visually represents the trade-offs between different error types. -->
...
<!-- Image Description: The image is a flowchart illustrating a No-Free-Lunch theorem within a unified federated learning framework. It shows how efficiency reduction (€ₑ), Bayesian privacy leakage (€ₚ), and utility loss (€ᵤ) contribute to a central theorem (4.6). This theorem, expressed as an inequality, is then used in an optimization problem (Sect. 3) and applied to various cryptographic techniques (randomization, Paillier, secret sharing, compression), each supported by a separate theorem (5.1-5.4). The diagram visually connects these concepts and their mathematical relationships. -->
