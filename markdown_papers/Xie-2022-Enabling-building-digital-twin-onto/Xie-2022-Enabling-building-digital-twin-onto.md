...
<!-- Image Description: This flowchart organizes industry data models and reference data libraries/dictionaries across three sectors: manufactured things (automotive/aerospace), process plants, and things in the landscape (cities, buildings, infrastructure). Each sector lists relevant standards, ontologies, and data models (e.g., ISO 10303, Building Smart, INSPIRE), illustrating the data sources and interoperability challenges within each domain. The chart's purpose is to provide a comprehensive overview of existing data resources relevant to the paper's topic. -->
...
<!-- Image Description: This diagram illustrates a data architecture. A data warehouse feeds an ETL (Extract, Transform, Load) process. The ETL process populates two data source groups: "Pseudo-static" (e.g., BIM data) and "Dynamic" (e.g., BMS and IoT data). Wrappers interface these sources with a mediator that handles on-demand user requests. The architecture emphasizes data integration and flexible access for different data types. -->
...
<!-- Image Description: This image is a directed graph depicting an ontology, likely for a building information model (BIM). Nodes represent concepts (e.g., "brick:Point," "bot:Zone") and edges represent relationships between them (e.g., "brick:hasPoint," "bot:hasElement"). The graph illustrates how different elements, such as zones, points, equipment, and interfaces, relate within the ontology. Self-loops on some nodes indicate reflexive relationships. The inclusion of "3D model" suggests the ontology's link to visual representations. -->
...
<!-- Image Description: The image displays a building's 3D model (a) and its knowledge graph representation (b). (a) shows a top-down and perspective view of "Alan Reece building", highlighting "Seminar Room 3" and its location relative to other rooms and corridors, and sensors within the room. (b) presents a graph depicting relationships between building elements (rooms, sensors, lighting systems) using RDF triples, showing how they are connected in a semantic model. The graph illustrates the interconnectedness of physical spaces and their associated devices using ontologies. -->
