...
<!-- Image Description: This diagram illustrates a personal knowledge graph. A central "User" node connects to various nodes representing personal connections (Mom, Jamie, hometown, high school) and interests (guitars). Dashed lines link to external knowledge graphs: a general-purpose KG and domain-specific KGs (e-commerce catalog, social network). The image depicts how personal knowledge is interconnected with broader knowledge bases. -->
...
<!-- Image Description: The image displays three example conversational exchanges between a user and an AI assistant. Each example (a, b, c) is a multi-turn dialogue represented as a sequence of chat bubbles, color-coded to distinguish user and AI messages. The figure illustrates different contexts: (a) purchasing guitar strings, (b) scheduling a dentist appointment, and (c) planning a running route. The purpose is to showcase the AI's ability to engage in diverse conversational tasks and provide helpful responses. -->
