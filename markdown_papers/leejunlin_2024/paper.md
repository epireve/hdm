...
<!-- Image Description: The image compares two question-answering frameworks. (a) shows a knowledge graph (KG)-augmented framework, where a question about Kobe Bryant's team colors is answered incorrectly as "yellow". (b) presents a multimodal reasoning framework (MR-MKG) that incorporates an image of a black mamba.  This framework correctly identifies the Los Angeles Lakers' colors ("purple, gold") by linking the image, the player's alias, and the team via the KG.  Both diagrams illustrate the question, prompt, LLM processing, and generated answers.  The difference highlights MR-MKG's improved accuracy through multimodal input. -->
...
<!-- Image Description: This image depicts a multimodal model architecture for question answering.  It shows three input streams: text (a question and a knowledge graph), an image, and an LLM.  These are processed through separate encoders and adapters before converging. The model uses a contrastive loss function (generative and alignment loss) for cross-modal alignment, optimizing for accurate answers based on image and knowledge graph inputs.  The diagram visually illustrates the data flow and loss function components within the model. -->
...
<!-- Image Description: The image contains two line graphs comparing the performance of two models, ScienceQA and MARS, across varying numbers of knowledge pieces.  The left graph shows ScienceQA's average accuracy (%), peaking around 20 knowledge pieces. The right graph displays MARS's Hits@1 (%) metric, also showing a peak before declining.  Both graphs illustrate the relationship between the quantity of knowledge and model performance. -->

Figure 3: Impact of numbers of knowledge triplets.

<span id="page-7-3"></span>![](_page_7_Figure_10.jpeg)
<!-- Image Description: The image contains two line graphs comparing the performance of a model (likely a graph neural network) with varying numbers of RGAT layers on two datasets: ScienceQA and MARS.  The left graph shows average accuracy (%) on ScienceQA, peaking at 93% with 2 layers then declining slightly.  The right graph displays Hits@1 (%) on MARS, reaching a maximum of around 40% at 3 layers before dropping.  The purpose is to illustrate the effect of the number of RGAT layers on model performance across different datasets. -->
...
<!-- Image Description: The image presents two cases illustrating a model's performance on knowledge graph reasoning tasks.  Case A (MARS Task) shows a knowledge graph representing a combustion reaction, using images and relations like "has use" and "subclass of" to predict the use of coal. Case B (ScienceQA Task) depicts a geographical knowledge graph, using state maps and relations such as "adjoin" to answer a question about a highlighted US state. Both cases demonstrate the model's ability to integrate visual and relational information for prediction, showing correct (MR-MKG) and incorrect (No KG) answers. -->
...
<!-- Image Description: The image displays a visual question answering (VQA) system.  Two street scenes are shown, each with bounding boxes around people.  Below each image is a knowledge graph representing the scene's objects and their relationships (e.g., "man wearing jacket," "behind"). The graphs illustrate how the system infers answers to questions like "What is behind and to the right of the people talking?"  The improved graph (bottom) incorporates image attributes like "tall" and "green" for more accurate responses.  The system uses both image data and relational reasoning to achieve visual understanding and answer questions. -->
...
<!-- Image Description: The image displays a knowledge graph (KG) depicting relationships between concepts (e.g., "water," "rock," "milk").  Nodes represent concepts, and edges represent relationships like "part of," "used by," or "subclass of."  Images of water, milk, and a rock illustrate the concepts. The graph is used to answer a question selecting the solid object from a list of choices (water, rock, milk), demonstrating the KG's reasoning capabilities. The model prediction highlights the correct answer ("rock"). -->
...
<!-- Image Description: Figure 8 presents two examples of a Multi-modal Relational Semantic model (MARS).  The first demonstrates analogical reasoning:  given an image of soy milk and soybeans, the model correctly infers the "made from material" relationship. The second example shows knowledge retrieval: given an image of a largemouth bass found in Lake Michigan,  the model correctly identifies its kingdom ("animal") and links it to related film genres ("comedy") via the movie *Big Fish*.  Both examples illustrate the model's ability to integrate visual and textual information for knowledge representation and reasoning. -->
