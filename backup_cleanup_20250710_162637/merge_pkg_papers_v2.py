...
Version 2 - Better handling of multi-line CSV format
"""
...
def parse_multiline_csv_improved(file_path):
    """Parse the complex multi-line CSV format more accurately"""
    papers = []
    ...
    # Better CSV parsing for headers
    ...
    while i < len(lines):
        ...
        # New entries typically have a non-empty first field (paper title)
        # and contain certain keywords or patterns
        
        # Parse the line to check first field
        try:
            reader = csv.reader([line])
            fields = next(reader)
            
            # Check if first field is non-empty and looks like a paper title
            if len(fields) > 0 and fields[0].strip():
                first_field = fields[0].strip()
                
                # Check if it's actually a paper title (not a continuation)
                if (len(first_field) > 10 and 
                    not first_field.startswith('The ') and
                    not first_field.startswith('A ') and
                    'Paper Title' not in first_field and
                    not all(c in '.,;:' for c in first_field)):
                    
                    # Process previous record if exists
                    if current_record:
                        paper = process_paper_record(current_record, headers)
                        if paper and (paper.get('Papers') or paper.get('Authors')):
                            papers.append(paper)
            ...
            # If CSV parsing fails, just add to current record
            current_record.append(line)
...
def process_paper_record(record_lines, headers):
    """Process a multi-line paper record into a dictionary"""
    if not record_lines:
        return None
    ...
    # Use csv reader to parse the combined text
    ...
    for i, header in enumerate(headers):
        if i < len(all_fields):
            value = all_fields[i].strip()
            if value and value != ',':
                paper[header] = value
    
    # Special handling for Papers (title) field
    if not paper.get('Papers'):
        # Try to extract from first non-empty field
        for field in all_fields:
            if field and len(field) > 20 and not field.startswith(','):
                paper['Papers'] = field.strip()
                break
...
def fuzzy_match(str1, str2, threshold=0.8):
    ...
    # Remove common academic prefixes/suffixes and special characters
    for pattern in [r'\s*:\s*', r'\s*-\s*', r'^(a|an|the)\s+', r'\s*\(.*?\)\s*$', r'[^\w\s]']:
...
def match_papers(primary_papers, secondary_papers):
    ...
    for sec_paper in secondary_papers:
        ...
        for prim_idx, prim_paper in enumerate(primary_papers):
            ...
            if sec_authors and prim_authors:
                ...
                for sec_auth in sec_author_list:
                    for prim_auth in prim_author_list:
                        if fuzzy_match(sec_auth, prim_auth) > 0.8:
                            author_score = 0.8
                            break
                    if author_score > 0:
                        break
            ...
            combined_score = (title_score * 0.8) + (author_score * 0.2)
...
def main():
    ...
    # Parse secondary dataset with improved parser
    secondary_papers = parse_multiline_csv_improved(secondary_file)
    print(f"Extracted {len(secondary_papers)} papers from secondary dataset")
    
    # Debug: Show first few papers
    print("\nFirst 3 papers from secondary dataset:")
