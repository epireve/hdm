...
class HDMPDFConverter:
    ...
    def setup_paths(self):
        ...
        if self.args.output_dir:
            ...
            # Always use markdown_papers as the output directory
            self.output_dir = BASE_DIR / "markdown_papers"
        ...
        # Checkpoint file - always use same name for consistency
        self.checkpoint_file = BASE_DIR / "checkpoint_markdown_papers.json"
    ...
    def convert_pdf_dual_mode(self, pdf_path: Path, worker_id: int) -> Dict:
        ...
        try:
            ...
            if not desc_success:
                self.logger.error(f"Description generation failed: {desc_output[:200]}")
            ...
            if not img_success:
                self.logger.error(f"Image extraction failed: {img_output[:200]}")
            ...
            # Don't cleanup if we want to debug
            if not self.args.debug:
                shutil.rmtree(temp_base, ignore_errors=True)
            else:
                self.logger.debug(f"Keeping temp directory for debugging: {temp_base}")
    ...
    def merge_dual_outputs(self, pdf_path: Path, desc_dir: Path, img_dir: Path) -> Optional[Dict]:
        ...
        try:
            # Find markdown files - marker creates them in subdirectories
            desc_md = list(desc_dir.rglob(f"{pdf_path.stem}.md"))
            img_md = list(img_dir.rglob(f"{pdf_path.stem}.md"))
            
            if not desc_md or not img_md:
                self.logger.error(f"Could not find markdown files to merge for {pdf_path.stem}")
                self.logger.debug(f"Looking in: {desc_dir} and {img_dir}")
                self.logger.debug(f"Found desc files: {[str(f) for f in desc_dir.rglob('*.md')]}")
                self.logger.debug(f"Found img files: {[str(f) for f in img_dir.rglob('*.md')]}")
                return None
            ...
            self.logger.info(f"✓ Saved markdown to: {final_md}")
    ...
    def show_progress(self, current: int, total: int):
        ...
        status_line = f'\r[{bar}] {percent:.1f}% | {current}/{total} PDFs'
        status_line += f' | {rate:.1f} PDFs/s | ETA: {int(eta)}s'
        status_line += f' | Mode: {self.args.mode.upper()}'
    ...
    def show_summary(self, results: List[Dict]):
        ...
        if self.stats['completed'] > 0:
            markdown_files = list(self.output_dir.rglob('*.md'))
            if markdown_files:
                print(f"\nMarkdown files saved: {len(markdown_files)}")
                print("Recent conversions:")
                for md_file in sorted(markdown_files, key=lambda x: x.stat().st_mtime)[-5:]:
                    print(f"  - {md_file.relative_to(self.output_dir)}")
