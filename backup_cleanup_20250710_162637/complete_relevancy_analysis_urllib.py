...
using KiloCode API (OpenRouter proxy) with Claude Sonnet 4 model.
This version uses urllib instead of requests to avoid dependency issues.
"""
...
class RelevancyAnalyzer:
    ...
    def make_api_request(self, prompt: str, max_tokens: int = 300) -> Optional[str]:
        ...
        req = urllib.request.Request(
            self.base_url, data=json.dumps(data).encode("utf-8"), headers=self.headers
        )
        ...
        try:
            ...
            if e.code == 429:  # Rate limit
                ...
                time.sleep(60)
                return self.make_api_request(prompt, max_tokens)  # Retry
            return None
...
Provide ONLY the relevancy level (SUPER, HIGH, MEDIUM, or LOW) as a single word response."""
        ...
        result = self.make_api_request(prompt, max_tokens=50)
        if result:
...
Provide a 2-3 sentence justification explaining why this paper received this relevancy rating. Focus on specific aspects of the paper that align or don't align with our research focus on heterogeneous data integration in PKG architectures. Be specific and reference actual content from the paper."""
        ...
        return self.make_api_request(prompt, max_tokens=300)

    def process_papers(
        self, input_file: str, output_file: str, test_mode: bool = False
    ):
        """Process papers to complete relevancy and justification"""
        # Read all papers
        with open(input_file, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            all_papers = list(reader)  # Keep all papers for final output
        ...
        # Test mode - process 5 papers that need processing
        if test_mode:
            ...
            for paper in papers:
                ...
                if not rel or not just or just.lower() == "not available":
                    papers_needing_processing.append(paper)
                if len(papers_needing_processing) >= 5:
                    break
            papers = papers_needing_processing
            logging.info(
                f"TEST MODE: Processing {len(papers)} papers that need relevancy/justification"
            )

        # Count papers needing processing
        papers_needing_processing = 0
        for paper in papers:
            rel = paper.get("Relevancy", "").strip()
            just = paper.get("Relevancy Justification", "").strip()
            if not rel or not just or just.lower() in ["", "none", "null", "not available"]:
                papers_needing_processing += 1
        
        logging.info(f"Starting processing from index {start_index}")
        logging.info(f"Total papers in dataset: {total_papers}")
        logging.info(f"Papers needing processing: {papers_needing_processing}")
        logging.info(f"Papers already processed in previous runs: {len(self.checkpoint_data['processed'])}")
        ...
        for i, paper in enumerate(papers):
            ...
            needs_justification = not justification or justification.lower() in [
                "",
                "none",
                "null",
                "not available",
            ...
            if not needs_relevancy and not needs_justification:
                logging.info(
                    f"Paper {cite_key} already has both relevancy and justification"
                )
                skipped_count += 1
                continue

            # Calculate progress percentage
            progress_percentage = ((processed_count + skipped_count + error_count) / papers_needing_processing * 100) if papers_needing_processing > 0 else 0
            logging.info(f"Processing paper {i+1}/{total_papers}: {cite_key} [{progress_percentage:.1f}% complete]")
            ...
            try:
                ...
                if needs_justification and relevancy:
                    ...
                    if new_justification:
                        ...
                        logging.error(
                            f"  - Failed to generate justification for {cite_key}"
                        )
                        error_count += 1
                        continue
                ...
                # Save checkpoint and show progress every 10 papers
                if processed_count % 10 == 0:
                    self.save_checkpoint()
                    overall_progress = ((processed_count + skipped_count + error_count) / papers_needing_processing * 100) if papers_needing_processing > 0 else 0
                    logging.info(f"=== Progress Update ===")
                    logging.info(f"Checkpoint saved. Processed: {processed_count}")
                    logging.info(f"Overall progress: {overall_progress:.1f}%")
                    logging.info(f"Estimated papers remaining: {papers_needing_processing - (processed_count + skipped_count + error_count)}")
                    logging.info(f"======================")
        ...
        # Update the all_papers list with processed data
        if not test_mode:
            # Create a mapping of processed papers
            processed_map = {p['cite_key']: p for p in papers if p.get('cite_key') in self.checkpoint_data['processed']}
            
            # Update all_papers with processed data
            for i, paper in enumerate(all_papers):
                if paper['cite_key'] in processed_map:
                    all_papers[i]['Relevancy'] = processed_map[paper['cite_key']].get('Relevancy', paper.get('Relevancy', ''))
                    all_papers[i]['Relevancy Justification'] = processed_map[paper['cite_key']].get('Relevancy Justification', paper.get('Relevancy Justification', ''))
        
        # Write ALL papers (complete dataset)
        papers_to_write = papers if test_mode else all_papers
...
def main():
    ...
    project_root = Path(__file__).parent.parent.parent
    input_file = str(project_root / "research_papers_complete.csv")
    output_file = str(
        project_root
        / (
            "research_papers_complete_updated.csv"
            if not test_mode
            else "research_papers_test_output.csv"
        )
    )
