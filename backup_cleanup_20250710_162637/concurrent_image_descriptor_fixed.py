#!/usr/bin/env python3
"""
Concurrent image descriptor that processes multiple papers in parallel
Fixed version that properly escapes backslashes in descriptions
"""
import os
import re
import time
    ...
    try:
        ...
        for image_file in images_to_process:
            ...
            try:
                ...
                IMPORTANT: Do not use backslashes or LaTeX notation in your description."""
                ...
                if response.text:
                    ...
                    # Find the exact image reference and add description comment after it
                    # Use a more robust replacement method
                    image_ref = f'![]({image_file})'
                    if image_ref in updated_content and not has_image_description(updated_content, image_file):
                        # Direct string replacement instead of regex
                        new_text = f'{image_ref}\n<!-- Image Description: {description} -->'
                        updated_content = updated_content.replace(image_ref, new_text, 1)
                        images_described += 1
                ...
                thread_print(f"[Worker {worker_id}] Error with image {image_file} in {paper_name}: {str(e)}")
...
def main():
    ...
    num_workers = min(len(API_KEYS), 3)  # Use up to 3 workers
    
    print(f"Concurrent Image Descriptor (Fixed)")
    print(f"="*60)
