...
![This flowchart illustrates a data aggregation system. Three data sources—web/internet, sensor networks, and mobile/wearable devices—feed into an aggregation stage. The aggregated data is then categorized into public (news, weather, traffic) and personal data, which are subsequently used for personal and social applications. The diagram visually depicts the data flow and categorization within the system.](_page_0_Figure_23.jpeg)
...
The author previously proposed an organizational structure, especially for visual exploration of data [[9]](#ref-9). In this research, the collected data include the following.
...
- 1. Common metadata and schema matching method for aggregation of different schemas from various information sources [[11]](#ref-11)
...
Information visualization and interactive user interfaces can help reveal buried facts and relationships among data [[2]](#ref-2). An effective navigation system is essential to enable interaction with a large amount of data from a user's point of view. Therefore, a navigational structure and a temporal zooming user interfaces were proposed for heterogeneous data [[9]](#ref-9).

Three viewpoints were studied for organizing data: time (when), location (where), and people (who). The time axis was treated as a major attribute for the organization of data. We have also studied summaries and landmarks to visualize a large amount of data [[9]](#ref-9). Summaries are like digests of daily life, and landmarks provide cues for remembering experiences. Usually landmarks are selected from personal data. Of course, summaries and landmarks change depending on the viewpoints and their scale. The change in scale for each viewpoint corresponds to changes in time (time), geographical region (location), and groups of people (people). We studied six types of landmarks, user-generated data, locations, people, tags, characteristic values (e.g., outliers), and public data (e.g., news headlines) [[9]](#ref-9). The summary contains these landmarks.

![The image is a hierarchical diagram illustrating different "digest views" of personal and landmark data over time. The top level shows a yearly view, broken down into monthly views, further subdivided into daily views. Black circles represent personal data points, and red stars represent landmark data points, distributed across the time axis. The diagram visually demonstrates a data aggregation approach, showing how data is summarized at different granularities (yearly, monthly, daily).](_page_1_Figure_18.jpeg)
**Figure 2. Navigation structure**![This diagram illustrates a data visualization system for personal data. Three nested views—daily, monthly, and yearly—are depicted as three-dimensional arrays of data blocks. Zooming in/out transitions between these views. Navigation is shown using arrows indicating the movement between views and the time relationships between them. The "landmark" concept indicates the user's current position within the dataset, facilitating data exploration across different time granularities.](_page_1_Figure_20.jpeg)
...
![The diagram illustrates a system architecture for data collection and analysis using smart devices. Data is collected from various sources, including the WWW (e.g., Flickr, Twitter), sensor networks (e.g., home energy use), and mobile devices (GPS data, call history). This data undergoes mining, producing metadata that feeds a request handler accessible by clients (smartphones/tablets). The system's components are represented by boxes and cylinders, depicting data flow and processing stages.](_page_2_Figure_1.jpeg)
**Figure 4. Prototype system**![This image from an academic paper displays a tablet screen showing a user interface. The interface presents a collage of photos labeled "Landmark photo" and "Representative locations," a bar graph illustrating "History of number of steps walked per month," and a tag cloud of keywords ("Tag clouds (landmark tags)"). A news feed is visible on the right, and the home energy cost is displayed (4865円). The image likely illustrates the data visualization and user experience aspects of a location-based application or system.](_page_2_Picture_3.jpeg)
...
![The image displays a tablet screen showing a time series graph of gas consumption (presumably) over a month in May 2010. A prominent spike, labeled "Outlier," is highlighted. The graph is part of a larger application interface that also shows news feeds and other data. The image's purpose is likely to illustrate an outlier detection method within the context of the paper, possibly for anomaly detection in time-series data. A bar chart is also visible alongside the graph, presenting another data point.](_page_2_Figure_7.jpeg)
...
MyLifeBits is a popular research project for storing data of daily activities in a database [[5]](#ref-5). It stores data from PCs and photos taken with a SenseCam, which is a mobile device that has a camera module, digital light sensor, and a temperature sensor [[6]](#ref-6). On the other hand, we focused on integrating personal data that have been stored on various sources without using special devices.

Ringel et al. worked with personal and public landmarks [[8]](#ref-8). In their research, personal landmarks were important calendar events and the first photos taken on given days. Public landmarks were national holidays and important news events. Only files on PCs were used, and the time scale for the landmarks was fixed. In our approach, several
...
![The image is a graph illustrating a data model. Nodes represent data points (people, locations, dates, tags), colored differently to indicate data type. Edges connect related data, showing relationships between different data attributes within a network or system. The central node suggests a core record linked to various metadata. The graph visually depicts the structure of interconnected data elements, crucial for understanding data organization in the paper's context.](_page_3_Figure_12.jpeg)
...
![The image displays a user interface showing a visual representation of data. A central image, seemingly a photograph of illuminated Christmas decorations, is connected to nodes representing date, location (a Google Map snippet), tags (text in Japanese), and related people (NONE). A right-hand panel lists associated photos with timestamps and descriptive tags. The diagram illustrates a data organization method, likely for a multimedia database or similar application, linking different data points through visual connections.](_page_3_Figure_15.jpeg)
...
![This image from an academic paper displays a visual representation of data linked to a person, Natsumi Teraoka. A central SMS message ("終わった") is connected to metadata including date, location (shown via a Google Map snippet), and related images (a list of photo files). The diagram illustrates how different data points, such as the SMS, date, location, and associated photos, are interconnected and used to reconstruct events. It highlights the connection between digital data and personal information.](_page_4_Figure_0.jpeg)
...
Agarwal et al. presented a system that could reconstruct 3D scene from extremely large collections of photographs on Internet photo sharing sites [[13]](#ref-13). They also showed examples of reconstructions of several city data sets from Flickr.
...
Typical activity recommendations are based on location using GPS history [[3]](#ref-3), [[10]](#ref-10). For personal use, location and activity (e.g., shopping and watching movies) recommendations are effective. However, these researches were not used various heterogeneous data.

Several practical applications can be envisaged, such as predicting traffic jams, detecting popular trends, and monitoring public health [[4]](#ref-4), in terms of public use of heterogeneous data. Of course, such recommendations are important topics for future work.

Eagle et al. proposed 'reality mining' system that measured information access and use within different contexts, recognized social patterns in daily user activities, and inferred relationships [[14]](#ref-14). On the other hand, we have studied relations among heterogeneous data with respect to attributes from the user's viewpoint.
...
- <a id="ref-1"></a>1. G. Bell and J. Gemmell. Total Recall. DUTTON, 2009.
- <a id="ref-2"></a>2. D. A. Keim. Information visualization and visual data mining. IEEE Trans Visualization and Computer Graphics, 8(1):1–8, 2002.
- <a id="ref-3"></a>3. D. Dearman and K. N. Truong. Identifying the Activities Supported by Locations with Community-Authored Content. In Proc. UbiComp'10, 23–32. ACM Press, 2010.
- <a id="ref-4"></a>4. D. Zhang, B. Guo and Z. Yu. Social and community intelligence. IEEE Computer (to appear), 2011.
- <a id="ref-5"></a>5. J. Gemmell, G. Bell, and R. Lueder. Mylifebits: A personal database for everything. Communications of the ACM, 49(1):88–95, 2006.
- <a id="ref-6"></a>6. J. Gemmell, L. Williams, K. Wood, R. Lueder, and G. Bell. Passive capture and ensuing issues for a personal lifetime store. In Proc. CARPE '04. ACM, 2004.
- <a id="ref-7"></a>7. M. Cammarano, X. Dong, B. Chan, J. Kinger, J. Talbot, A. Halevy and P. Hanrahan. Visualization of heterogeneous data. IEEE Trans Visualization and Computer Graphics, 13(6):1200–1207, 2007.
- <a id="ref-8"></a>8. M. Ringel, E. Cutrell, S. Dumais, and E. Horvitz. Milestones in time. In Proc. INTERACT 03. IFIP, 2003.
- <a id="ref-9"></a>9. T. Teraoka. A Study of Exploration of Heterogeneous Personal Data Collected from Mobile Devices and Web

Services. In Proc. MUE2011, 239-245. IEEE CPS, 2011.

- <a id="ref-10"></a>10. V. W. Zheng, Y. Zheng, X. Xie and Q. Yang. Collaborative location and activity recommendations with gps history data. In Proc. WWW2010, 1029-1038. ACM, 2010.
- <a id="ref-11"></a>11. W. Xue, H. Pung, P. P. Palmes and T. Gu. Schema Matching for Context-Aware Computing. In Proc. UbiComp'08, 292–301. ACM Press, 2008.
- <a id="ref-12"></a>12. Y-ah Kang, C. Gorg and J. Stasko. How can visual analytics assist investigative analysis? Design

implications from an evaluation. IEEE Trans Visualization and Computer Graphics, 17(5):570–583, 2011.

- <a id="ref-13"></a>13. S. Agarwal, N. Snavely, I. Simon, S. M. Seitz and R. Szeliski. Building Rome in a day. In Proc. ICCV, 72- 79, IEEE, 2009.
- <a id="ref-14"></a>14. N. Eagle and A. Pentland. Reality mining: sensing complex social systems. Personal and Ubiquitous Computing, 10(4): 255-268, 2006
